########################################################################################
# https://github.com/dexter74/Archives/tree/main/Docker/V1/2.Conteneurs

########################################################################################
# nano ~/APPZ/ASUSTOR/AppCentral/NordVPN/CONTROL/post-install.sh;

########################################################################################
cd ~/APPZ/ASUSTOR/AppCentral/AdGuardHome   		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Bitwarden     		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Cloud9       		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Dozzle       		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Emby         		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/FileBrowser 		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/LibreSpeed		 	&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/MariaDB      		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Nexclipper    		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/NginxReverseProxy   	&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Plex          		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/phpMyAdmin    		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Portainer     		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Pyload/CONTROL 	&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/QBitorrent    		&& apkg-tools_py2.py create .
cd ~/APPZ/ASUSTOR/AppCentral/Visual_Studio 		&& apkg-tools_py2.py create .
########################################################################################

#
########################################################################################
# En Developpement
# cd ~/APPZ/ASUSTOR/AppCentral/Jackett      && apkg-tools_py2.py create .
# cd ~/APPZ/ASUSTOR/AppCentral/Jdownloader  && apkg-tools_py2.py create .
# cd ~/APPZ/ASUSTOR/AppCentral/Radarr       && apkg-tools_py2.py create .
# cd ~/APPZ/ASUSTOR/AppCentral/Sonarr       && apkg-tools_py2.py create .

########################################################################################
# Tentative KO
# cd ~/APPZ/ASUSTOR/AppCentral/NordVPN		&& apkg-tools_py2.py create .
# cd ~/APPZ/ASUSTOR/AppCentral/Wireguard	&& apkg-tools_py2.py create .
